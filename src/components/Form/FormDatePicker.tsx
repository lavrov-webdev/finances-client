...
export const FormDatePicker: FC<Props> = ({ name, withNextDayButton, withPrevDayButton, ...rest }) => {
    const form = useFormContext()
    useWatch({ control: form.control, name })

    const handleNextDayButtonClick = () => {
        const currentValue = dateTime({ input: form.getValues(name), format: DATE_FORMAT })
    ...
    return (
        <Controller
            ...
            render={({ field, fieldState }) => <>
                {withPrevDayButton && <Button view='flat' className={styles.buttonPrev} onClick={handlePrevDayButtonClick}>
                ...
                <DatePicker
                    ...
                    format={DATE_FORMAT}
                />
                {withNextDayButton && <Button view='flat' className={styles.buttonNext} onClick={handleNextDayButtonClick}>
