...
export const SprintsList = () => {
    ...
    const sprintsByYears = useMemo(() => groupSprintsByStartYear(sprintsState.data || []), [sprintsState.data])
    useErrorNotifier(sprintsState.error as AxiosError, 'Ошибка загрузки спринтов')
    ...
    if (sprintsState.isLoading) {
        ...
        content = Object.entries(sprintsByYears).map(([year, sprints]) => {
            return <div className={styles.yearBlock} key={year}>
                <Text className={styles.yearTitle} variant="subheader-2">{year}</Text>
                <div className={styles.sprintsGrid}>
                    {
                        sprints.map(sprint => <Link
                            key={sprint.id}
                        ...
                        >
                            <GUICard className={styles.sprintsItemCard}>
                                {fomratSprintsDates(sprint.startDate)}-{fomratSprintsDates(sprint.endDate)}
                            </GUICard>
                        </Link>)
                    }
                </div>
