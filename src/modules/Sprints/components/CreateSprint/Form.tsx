...
import { useCreateSprint } from "@modules/Sprints/api/hooks/useCreateSprint"
import { CreateSprintDto, TCreateSprintDto, TCreateSprintDtoFields } from "@modules/Sprints/types"
...
export const Form = () => {
    const form = useForm<TCreateSprintDtoFields, any, TCreateSprintDto>({
        resolver: zodResolver(CreateSprintDto), defaultValues: {
            envelopes: [],
            startSum: 0,
    ...
    useEffect(() => {
        form.setValue("envelopes", categoriesState.data?.reduce<TCreateEnvelopeDto[]>((prev, category) => {
        ...
        <form className={styles.form} onSubmit={form.handleSubmit(onSubmit)}>
            <FormProvider {...form}>
                <Card title="Начать новый спринт">
                    ...
                    <StartSum />
                    <Envelopes />
                    <Button loading={createSprintMutate.isPending} type="submit" view="action">Создать</Button>
                </Card>
                <Preview />
            </FormProvider>
        </form>
