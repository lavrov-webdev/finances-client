...
import { useFormatAmount, useModal } from "@system/hooks"
...
export const Summary: FC<Props> = ({ sprintId }) => {
    ...
    const initialPlannedExpense = useFormatAmount(sprintSummary.initialPlannedExpense)
    const initialPlannedRemaining = useFormatAmount(sprintSummary.initialPlannedRemaining)
    ...
    const actualPlannedRemaining = useFormatAmount(sprintSummary.actualPlannedRemaining)
        ...
        <Card
            ...
            actions={<EditSprintModal sprintId={sprintId} />}>
            {
                sprintState.isLoading ? (
                    ...
                    <>
                        <Definition title="Текущий баланс" value={currentBalance} withDivider />
                        <Definition title="Стартовая сумма" value={initialAmount} withDivider />
                        <Definition title="Стартовый план" value={initialPlannedExpense} withDivider />
                        <Definition title="Плановый остаток на старте" value={initialPlannedRemaining} withDivider />
                        <Definition title="Общие расходы" value={totalExpenses} withDivider />
                        <Definition title="Осталось по плану" value={remainingBudget} withDivider />
                        <Definition title="Фактический плановый остаток" value={actualPlannedRemaining} />
                    </>
                )
            }
        </Card>
