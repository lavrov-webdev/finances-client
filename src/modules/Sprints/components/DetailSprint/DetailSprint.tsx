...
export const DetailSprint: FC<Props> = ({ sprintId }) => {
  ...
  useErrorNotifier(sprintState.error as AxiosError, 'Не удалось загрузить спринт')
  const selectedEnvelopeTransactions = useSelectedEnvelopeTransactions(sprintState.data?.envelopes || [], selectedEnvelopeId)
  ...
  return <div className={styles.container}>
    ...
    <Card title='Расходы' className={styles.transactions} maxWidth="none">
      {
        selectedEnvelopeId === null ? <div className={styles.withNoEnvelope}>
          <Text variant='subheader-3'>Выберите категорию в таблице</Text>
        </div> : <TransactionsTable
