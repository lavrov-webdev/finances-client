...
export const EditCategories = () => {
    ...
    const editableCategoriesArray = useFieldArray({ control: form.control, name: "editableCategories", keyName: "rhfId" })
    useWatch({ control: form.control, name: "editableCategories" })

    return (
        <div className={styles.cardContent}>
            <Text className={styles.subtitle} variant="subheader-3">Редактировать существующие</Text>
            {isLoading && <EditCategoriesSkeleton />}
            {(editableCategoriesArray.fields.length === 0) ?
                ("Пока у вас нет категорий") : (
                    <Flex direction="column" gap={3}>
                        {editableCategoriesArray.fields.map((category, id) => <EditableCategory
                            key={category.id}
