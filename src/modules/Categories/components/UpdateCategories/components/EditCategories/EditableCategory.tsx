...
export const EditableCategory: FC<Props> = ({ categoryId, id }) => {
    ...
    const isCategoryToDelte = useMemo(() => categoriesToDelete.has(categoryId), [categoryId, categoriesToDelete])

    const deleteButton = <Button onClick={addCategoryToDeleteList}>
        <Icon data={TrashBin} />
    </Button>
    const returnButton = <Button onClick={returnCategory}>
    ...
    return (
        <Flex height={28} gap={2} alignItems="center">
            ...
            /> : <div
                className={cn(
                    styles.editCategoriesEditableCategoryView,
            ...
            <div>
                {isCategoryToDelte ? returnButton : deleteButton}
            </div>
        </Flex>
    )
}