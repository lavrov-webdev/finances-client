...
import { DetailSpritnLoader, getCurrentSprintQueryOptions, getSprintByIdQueryOptions } from '@modules/Sprints'
...
function Component() {
    ...
    const sprintId = useMemo(() => currentSprint.data?.id?.toString(), [currentSprint.data])
    ...
    if (typeof sprintId === 'undefined') {
        return <Alert
            ...
            actions={
                <Alert.Action
                    onClick={() => navigate({ to: "/sprints/create" })}
                >
                    Создать
                </Alert.Action>}
        />
    }
    return currentSprint.isLoading ? <DetailSpritnLoader /> : <Navigate to="/sprints/$sprintId" params={{ sprintId }} />
}